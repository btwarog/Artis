...
abstract class CachedBookmarkedArtistsDao {
	...
	@Query("SELECT id FROM $BOOKMARKED_ARTISTS_TABLE_NAME WHERE id = :artistId")
	abstract suspend fun getBookmarkedArtist(artistId: String): Flow<List<String>>
	...
	abstract fun getAllBookmarkedArtistsWithIds(ids: List<String>): Flow<List<String>>

	@Query("DELETE FROM $BOOKMARKED_ARTISTS_TABLE_NAME WHERE id = :artistId")
	abstract suspend fun deleteBookmarkedArtist(artistId: String): Long
}